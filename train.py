...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=128,
                        help='Batch size.')
    parser.add_argument('--num_factors', type=int, default=8,  # todo : 不再使用embedding矩阵后，该参数或可删掉
                        help='Embedding size.')
    ...
    parser.add_argument('--label_numbers', type=int, default=96,
                        help='The total number of user labels.')
    parser.add_argument('--user_material_vector_length', type=int, default=70,
                        help='The length of user material sequence vector.')
    parser.add_argument('--full_connection_size', type=int, default=100,  # todo: the full connection size ?
                        help='The num of columns of the matrix in full connection layer.')
    parser.add_argument('--dish_sequence_length', type=int, default=40,
                        help='The length of dish sequence vector.')
    parser.add_argument('--vocab_size', type=int, default=1261,
                        help='The total number of all words./The length of vocabulary.')
    parser.add_argument('--embedding_size', type=int, default=128,
                        help='Size of embedding matrix.')
    parser.add_argument('--filter_sizes', nargs='?', default='[6, 7, 8]',
                        help='Filter sizes.')
    ...
    parser.add_argument('--high_level_score_coefficient', type=float, default=0.5,
                        help='The coefficient of high level score.')
    parser.add_argument('--low_level_score_coefficient', type=float, default=0.5,
...
if __name__ == '__main__':
    ...
    # model_out_dir = '../../../share/jianghao/GMF_checkpoint/'
    model_out_dir = '../../../share/wangwenjie/jianghao/Parameter/0.5-0.5'
    ...
    with tf.Session(config=config) as sess:
        ...
        model = Model(args.num_factors, eval(args.regs), args.num_neg, args.learner, args.lr, args.epochs,
                      args.batch_size, args.verbose, num_users, num_items, args.decay_steps, args.decay_rate,
                      args.label_numbers, args.user_material_vector_length, args.full_connection_size,
                      args.dish_sequence_length, args.vocab_size, args.embedding_size, eval(args.filter_sizes),
                      args.num_filters, args.num_categories, args.high_level_score_coefficient,
                      args.low_level_score_coefficient, Personal_Memory, Recipe_Embedding, Category_Embedding)
        ...
        for epoch in range(args.epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, args.batch_size),
                ...
                curr_loss, lr, logits,  _ = sess.run([model.loss_value, model.learning_rate, model.logits, model.train_op],
                                                     feed_dict)
